...
router.get('/fileuploads/:id', requireToken, (req, res) => {
...
router.patch('/fileuploads/:id', requireToken, (req, res) => {
  ...
  FileUpload.findById(req.params.id)
    ...
    .then(fileupload => {
      ...
      Object.keys(req.body.fileupload).forEach(key => {
        if (req.body.fileupload[key] === '') {
          delete req.body.fileupload[key]
        }
      })

      // pass the result of Mongoose's `.update` to the next `.then`
      return fileupload.update(req.body.fileupload)
    })
...
router.delete('/fileuploads/:id', requireToken, (req, res) => {
  FileUpload.findById(req.params.id)
    .then(handle404)
    .then(fileupload => {
